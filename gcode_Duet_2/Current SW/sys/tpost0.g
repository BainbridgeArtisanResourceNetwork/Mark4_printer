; tpost0.g
;!<summary>Actions to do with the new tool after it is selected.   tPostN is the third macro called in transitions fm one tool to another. tpreM.g is used to dock the previous tool (M). The tpreN is used to preheat the new tool (N). Finally tpost.N (this macro) is used to undock the tool and return to the starting position. 
